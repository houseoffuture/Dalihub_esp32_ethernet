...
-- Send and recieve DALI message 
     if dalitype then
      ...
      toResp = tcpdata['cmd']..' '..tcpdata['addtype']..' '..tcpdata['address']
...
server:listen(6006, function(connection)
--Wait for message
    connection:on("receive", function(connection, payload)
      res = main(payload)
      sep = "\r"
      gpio.write(4, 0)
      if string.match(payload, 'DTR') then connection:send('DTR is write!\r')
      else
      --if dalitype then sep = "\r" else sep = "End" end
      --if res == 1 then
        uart.on(2,"data", sep,
        --s:on("data", "\r",
        function(data)
        if payload == 'scan' or payload == 'ini' or payload == 'buslevel' then
          connection:send(data..'\r')
          gpio.write(4, 1)
        else
          dali_rx = string.match(data, '[+-]?%d+')
          uart.on(2,"data")
          connection:send(toResp..' '..dali_rx..'\r')
          gpio.write(4, 1)
        end
        end , 1)
      end 
